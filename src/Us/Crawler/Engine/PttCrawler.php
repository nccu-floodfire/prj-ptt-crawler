...
class PttCrawler
{
    ...
    private function main()
    {
        ...
        if (isset($this->config['start-page'])) {
            ...
            // 取得總頁數
            $last_page = $this->countPage();
            if ($last_page === null) {
                $this->errorOutput("Failed to get total page \n");
                return false;
            }
        }
        ...
        for ($i = $last_page; $i >= 1; $i--) {
            ...
            // 取得每頁文章基本資料
            $current_page = $this->fetchPage($i);

            // 過濾失敗文章
            if ($current_page == null) {
                $this->errorOutput("notice! list: " . $i . " was skipped \n");
                continue;
            }
            ...
            foreach ($current_page as $item) {
                ...
                if ($this->storage->getArticleByArticleId($article_id)) {
                    $this->errorOutput("notice! article: " . $article_id . " ({$item['date']}) has been in database \n");
                    // 檢查是否抓到上次最後一篇
                ...
                if ($this->isDateAhead($item["date"])) {
                    $this->errorOutput("notice! article: " . $item["url"] . " ({$item['date']}) is ahead " . $this->config["start-date"] . ", skipped \n");
                    $state |= self::STATE_DATE_AHEAD;
                    continue;
                }
                // 略過已到設定日期文章
                if ($this->isDateOver($item["date"]) && $i < ($last_page - 1)) {
                    // 頭兩頁不因為過期而停止
                    $this->errorOutput("notice! article: " . $item["url"] . " ({$item['date']} is earlier than " . $this->config["stop-date"] . " \n");
                    $is_stop = true;
            ...
            foreach ($save_article_arr as $item) {
                $this->errorOutput("fetching article id: " . $item["url"] . "({$item['date']}) \n");
                // 取得每筆文章詳細資料
                $article_array = $this->fetchArticle($item["url"]);
                // 過濾詭異文章
                if ($article_array == null) {
                    $this->errorOutput("notice! article: " . $item["url"] . " was skipped \n");
                    continue;
                }
        ...
        if ($state & self::STATE_DATE_REACHED) {
            $this->errorOutput("Stop fetching cause the article date are older than " . $this->config["stop-date"] . "\n");
        }

        // 檢查是否已重複
        if ($state & self::STATE_DUPLICATED) {
            $this->errorOutput("Stop fetching cause the articles are duplicated. \n");
        }

        $this->errorOutput("Fetch finished! \n");
        return true;
    }
    ...
    {
        $result = array();
        $dom = HtmlDomParser::str_get_html($this->fetchPageHtml(null), $lowercase = true, $forceTagsClosed = true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN = false);
        if (!$dom) {
            return null;
        }
        ...
        $this->errorOutput("total page: " . $last_page . "\n");
        return $last_page;
    }
    ...
    private function fetchPage($index)
    {
        ...
        $dom = HtmlDomParser::str_get_html($this->fetchPageHtml($index), $lowercase = true, $forceTagsClosed = true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN = false);
        // 如果取得資料失敗, 回傳NULL
        if ($dom == null) {
            return null;
        }
    ...
    // 取得當篇文章的html
    private function fetchArticleHtml($id)
    {
        $result = null;
        ...
        while ($error_count < 3 && ($result = @file_get_contents($url, false, $context)) == false) { // FIXME do not use @
            ...
            if ($response == "404") {
                $this->errorOutput("response 404..., this article will be skipped \n");
                break;
            } else {
                $this->errorOutput("connection error, retry... \n");
                sleep($this->config["error_sleep"]);
    ...
    {
        $dom = HtmlDomParser::str_get_html($this->fetchArticleHtml($id), $lowercase = true, $forceTagsClosed = true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN = false);
                ...
                if (!$is_comments_start) {
                    ...
                    if (in_array($comment_type_str, $valid_comments)) {
                        ...
                        if (count(array_intersect($expected_keys, $actual_keys)) !== 4) {
                            $this->errorOutput("Incorrect comment line: " . $line . "\n");
                        } else {
                            $comment_data = array(
    ...
    private function errorOutput($message)
    {
